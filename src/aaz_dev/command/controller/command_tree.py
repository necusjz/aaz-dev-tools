...
def to_limited_primitive(command_group: CMDSpecsCommandGroup):
    copied = CMDSpecsCommandGroup()
    copied.names = command_group.names
    copied.help = command_group.help
    copied.commands = {}
    copied.command_groups = {}
    if isinstance(command_group.command_groups, CMDSpecsCommandGroupDict):
        iterator = command_group.command_groups.raw_items()
    else:
        iterator = command_group.command_groups.items()
    for k, v in iterator:
        sub_cg = CMDSpecsCommandGroup()
        sub_cg.names = v.names
        sub_cg.commands = None
        sub_cg.command_groups = None
        if isinstance(v, CMDSpecsCommandGroup):
            sub_cg.help = v.help
            copied.command_groups[k] = sub_cg
        elif isinstance(v, CMDSpecsPartialCommandGroup):
            sub_cg.help = CMDHelp()
            sub_cg.help.short = v.short_help
            copied.command_groups[k] = sub_cg
    if isinstance(command_group.commands, CMDSpecsCommandDict):
        iterator = command_group.commands.raw_items()
    else:
        iterator = command_group.commands.items()
    for k, v in iterator:
        sub_command = CMDSpecsCommand()
        sub_command.names = v.names
        sub_command.versions = None
        if isinstance(v, CMDSpecsCommand):
            sub_command.help = v.help
            copied.commands[k] = sub_command
        elif isinstance(v, CMDSpecsPartialCommand):
            sub_command.help = CMDHelp()
            sub_command.help.short = v.short_help
            copied.commands[k] = sub_command
